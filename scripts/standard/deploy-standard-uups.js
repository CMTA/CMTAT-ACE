...
async function main() {
  ...
  const extraInformationAttributes = [
    '1234567890',
    [terms.name, terms.uri, terms.documentHash],
  ...
  const Factory = await ethers.getContractFactory('ComplianceTokenCMTATUUPSUpgradeable', deployer);
  const cmtat = await upgrades.deployProxy(
    Factory,
    [admin, ERC20Attributes, extraInformationAttributes, policyEngineAddress],
    {
      initializer: 'initialize',
  ...
  console.log('ComplianceTokenCMTATUUPSUpgradeable (proxy):   ', cmtatProxyAddress);
  console.log('ComplianceTokenCMTATUUPSUpgradeable (impl):    ', cmtatImplAddress);
}
