...
async function deployTransferValidationPolicy (policyEngineAddress, ownerAddress, ruleAddresses) {
  ...
  const configParams = ruleAddresses.length > 0
    ? abiCoder.encode(['address[]'], [ruleAddresses])
    : '0x'
...
describe('TransferValidationPolicy', function () {
  beforeEach(async function () {
    ...
    this.transferFromSelector = this.cmtat.interface.getFunction('transferFrom(address,address,uint256)').selector

    // Set extractor for transfer and transferFrom selectors on the policy engine
    await this.policyEngine.connect(this.admin).setExtractor(this.transferSelector, this.extractorAddress)
    await this.policyEngine.connect(this.admin).setExtractor(this.transferFromSelector, this.extractorAddress)
  ...
  describe('MaxAmountRule', function () {
    beforeEach(async function () {
      ...
      await this.policyEngine.connect(this.admin).addPolicy(
        this.cmtatAddress, this.transferSelector, this.transferPolicyAddress, TRANSFER_PARAM_NAMES
      )
      await this.policyEngine.connect(this.admin).addPolicy(
        this.cmtatAddress, this.transferFromSelector, this.transferPolicyAddress, TRANSFER_PARAM_NAMES
      )
    })
    ...
    describe('transfer', function () {
      it('should allow transfer within max amount', async function () {
        await expect(
          this.cmtat.connect(this.admin).transfer(this.address1.address, 50n)
        ).to.not.be.reverted
      ...
      it('should allow transfer at exact max amount', async function () {
        await expect(
          this.cmtat.connect(this.admin).transfer(this.address1.address, 100n)
        ).to.not.be.reverted
      ...
      it('should reject transfer exceeding max amount', async function () {
        await expect(
          this.cmtat.connect(this.admin).transfer(this.address1.address, 101n)
        ).to.be.reverted
      })
    })
    ...
    describe('transferFrom', function () {
      ...
      it('should allow transferFrom within max amount', async function () {
        await expect(
          this.cmtat.connect(this.address1).transferFrom(this.admin.address, this.address2.address, 50n)
        ).to.not.be.reverted
      ...
      it('should reject transferFrom exceeding max amount', async function () {
        await expect(
          this.cmtat.connect(this.address1).transferFrom(this.admin.address, this.address2.address, 101n)
        ).to.be.reverted
      })
    })
    ...
    describe('canTransfer', function () {
      it('should return true for transfer within max amount', async function () {
        expect(
          await this.cmtat.canTransfer(this.admin.address, this.address1.address, 50n)
        ).to.be.true
      })

      it('should return false for transfer exceeding max amount', async function () {
        expect(
          await this.cmtat.canTransfer(this.admin.address, this.address1.address, 101n)
        ).to.be.false
      })
    })
    ...
    describe('canTransferFrom', function () {
      it('should return true for transferFrom within max amount', async function () {
        expect(
          await this.cmtat.canTransferFrom(this.address1.address, this.admin.address, this.address2.address, 50n)
        ).to.be.true
      })

      it('should return false for transferFrom exceeding max amount', async function () {
        expect(
          await this.cmtat.canTransferFrom(this.address1.address, this.admin.address, this.address2.address, 101n)
        ).to.be.false
      })
    })
  })
  ...
  describe('RestrictedAddressRule', function () {
    beforeEach(async function () {
      ...
      this.restrictedRule = await ethers.deployContract('RestrictedAddressRule', [[this.address2.address]])
    ...
    describe('transfer', function () {
      ...
      it('should reject transfer to restricted address', async function () {
        await expect(
          this.cmtat.connect(this.admin).transfer(this.address2.address, 50n)
        ).to.be.reverted
      })
      ...
      it('should reject transfer from restricted address', async function () {
        ...
        await expect(
          this.cmtat.connect(this.address2).transfer(this.address1.address, 50n)
        ).to.be.reverted
      })
    ...
    describe('canTransfer', function () {
      ...
      it('should return false when recipient is restricted', async function () {
        expect(
          await this.cmtat.canTransfer(this.admin.address, this.address2.address, 50n)
        ).to.be.false
      })

      it('should return false when sender is restricted', async function () {
        expect(
          await this.cmtat.canTransfer(this.address2.address, this.address1.address, 50n)
        ).to.be.false
      })
    })
  })
  ...
  describe('Multiple rules (MaxAmount + RestrictedAddress)', function () {
    beforeEach(async function () {
      ...
      this.restrictedRule = await ethers.deployContract('RestrictedAddressRule', [[this.address3.address]])
    ...
    it('should reject transfer exceeding max amount even to unrestricted address', async function () {
      await expect(
        this.cmtat.connect(this.admin).transfer(this.address1.address, 201n)
      ).to.be.reverted
    })

    it('should reject transfer to restricted address even within max amount', async function () {
      await expect(
        this.cmtat.connect(this.admin).transfer(this.address3.address, 50n)
      ).to.be.reverted
    })
    ...
    it('canTransfer should return false when either rule rejects', async function () {
      // Exceeds max
      expect(
        await this.cmtat.canTransfer(this.admin.address, this.address1.address, 201n)
      ).to.be.false
      // Restricted recipient
      expect(
        await this.cmtat.canTransfer(this.admin.address, this.address3.address, 50n)
      ).to.be.false
    })
  ...
  describe('Policy management', function () {
    it('should work with no rules configured', async function () {
      ...
      await expect(
        this.cmtat.connect(this.admin).transfer(this.address1.address, 500n)
      ).to.not.be.reverted
    })
    ...
    it('should allow owner to update rules via setRules', async function () {
      ...
      await expect(
        this.cmtat.connect(this.admin).transfer(this.address2.address, 101n)
      ).to.be.reverted
  ...
  describe('No policy attached (defaultAllow = true)', function () {
    ...
    it('canTransfer should return true when no policy is registered', async function () {
      expect(
        await this.cmtat.canTransfer(this.admin.address, this.address1.address, 500n)
      ).to.be.true
    })
  })
  ...
  describe('canTransfer with module-level restrictions', function () {
    ...
    it('canTransfer should return false when sender is frozen', async function () {
      ...
      expect(
        await this.cmtat.canTransfer(this.address1.address, this.address2.address, 10n)
      ).to.be.false
    })
    ...
    it('canTransfer should return false when transfer exceeds active balance (frozen tokens)', async function () {
      ...
      await this.cmtat.connect(this.admin)['freezePartialTokens(address,uint256)'](this.address1.address, 40n)
      expect(
        await this.cmtat.canTransfer(this.address1.address, this.address2.address, 20n)
      ).to.be.false
    })
